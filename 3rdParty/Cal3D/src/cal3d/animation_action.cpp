...
{  
  setType(TYPE_ACTION);
	// JLA
	m_total_time = pCoreAnimation->getDuration();
...
bool CalAnimationAction::update(float deltaTime)
{
  ...
  if(getState() == STATE_STEADY)
  {
    ...
    if(!m_autoLock && getTime() >= m_total_time - m_delayOut)
    {
      setState(STATE_OUT);
    }
    // if the anim is supposed to stay locked on last keyframe, reset the time here.
    else if (m_autoLock && getTime() > m_total_time )
    {
  ...
  if(getState() == STATE_OUT)
  {
    ...
    if(getTime() < m_total_time)
    {
      setWeight((m_total_time - getTime()) / m_delayOut * m_weightTarget);
    }
    else
    {
...
// JLA
void CalAnimationAction::remove(float delayOut) {
	setState( STATE_OUT );
	m_weightTarget = getWeight();
  cancelled = true;
	// Not enough time to remove the animation in delayOut secs
	if (getTime() + delayOut > m_total_time) {
		m_delayOut = m_total_time - getTime();
	} else {
		m_total_time = getTime() + delayOut;
		m_delayOut = delayOut;
	}
}
