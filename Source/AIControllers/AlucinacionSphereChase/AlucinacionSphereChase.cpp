#include "AlucinacionSphereChase.h"
...
AlucinacionSphereChase::AlucinacionSphereChase(
	float aradius_chase
...
AlucinacionSphereChase::~AlucinacionSphereChase() {
	printDebugString( "Destroying AlucinacionSphereChase...\n" );
...
bool AlucinacionSphereChase::update(IntelligentObject &object, float deltaTime) {

	// Think
	think( object, deltaTime );

	// Eliminar las particulas de correr cuando se deja de correr
	if( status != HUNTING && prev_status == HUNTING ) {
		if( run_particle ) {
			run_particle->kill( true );
			run_particle = NULL;
		}
	...
	switch( status ) {
		...
		case HUNTING: {
			if( prev_status != status ) {
				...
				SoundPlayer::get( )->playSample( "alucinacion_chasing" );
...
float AlucinacionSphereChase::getRotationToFollowPlayer( IntelligentObject &object ) const {
...
void AlucinacionSphereChase::think( IntelligentObject &object, float deltaTime ) {
...
void AlucinacionSphereChase::checkHitPlayer( IntelligentObject &object ) {
	...
	if( distance_to_player <= radius_touch_player ) {
		...
		// Lanzamos un sonido
		SoundPlayer::get( )->playSample( "alucinacion_hit_player" );
		...
		// Ayuda dinámica
		if( getFirstLevelLogic().show_dh_first_time_player_is_hit_by_alucinacion ) {
			getFirstLevelLogic().show_dh_first_time_player_is_hit_by_alucinacion = false;
			IGameState::showDinamicHelp( DinamicHelp::PLANET1_PLAYER_IS_HIT_BY_ALUCINACION );
		}
...
void AlucinacionSphereChase::collision( IntelligentObject &int_object, const Object &object, CollisionType collision_type, const D3DXVECTOR3 &normal ) {
