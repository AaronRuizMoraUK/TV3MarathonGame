...
TimeOutNumberRender::~TimeOutNumberRender( ) {
	printDebugString("Destroying TimeOutNumberRender...\n");
}

void TimeOutNumberRender::print( float x, float y, float letter_width, float letter_height, const char *atxt ) {
...
void TimeOutNumberRender::printfClosing( float x, float y, float letter_width, float letter_height, const char *msg, ... ) {
	char txt[512];
	va_list ap;
	va_start(ap, msg);
	...
	printClosing( x, y, letter_width, letter_height, txt );
}

void TimeOutNumberRender::printClosing( float x, float y, float letter_width, float letter_height, const char *atxt ) {
	assert( mesh && shader && texture );
	assert( letter_width>=0 && letter_height>=0 );
	...
	if( strcmp( lastLetter.c_str(), txt ) != 0 )
		...
		show_time += 3.0f * (Game::shader_time - last_global_time);
	lastLetter = txt;
	last_global_time = Game::shader_time;
	show_time = clamp(show_time, 0.0f, 1.0f);

	// Set the width and the height of a letter in screen
	handleParam = shader_fx->GetParameterByName( NULL, "letter_screen_width" );
	if( handleParam )
		shader_fx->SetFloat( handleParam, letter_width * show_time );

	handleParam = shader_fx->GetParameterByName( NULL, "letter_screen_height" );
	if( handleParam )
		shader_fx->SetFloat( handleParam, letter_height * show_time );
