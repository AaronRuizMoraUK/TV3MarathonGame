...
InitFirstLevelState::~InitFirstLevelState() {
	printDebugString("Destroying InitFirstLevelState...\n");
}

void InitFirstLevelState::update( float deltaTime ) {
	// Update Camera
	...
	if( !Global::fade.isFinished( ) )
		...
		getState("first_level_state")->getFocus();
		return;
	}
}

void InitFirstLevelState::preRender( ) {
}

void InitFirstLevelState::renderWorld3D( ) {
...
void InitFirstLevelState::renderWorld2D( ) {
...
void InitFirstLevelState::renderPostProcessed2D( ) {
	renderStateEnableAlphaBlending( );
...
void InitFirstLevelState::renderGameUI( ) {
}

void InitFirstLevelState::postRender( ) {
#ifndef NDEBUG
	// Print FPS
	int py = 0;
	py += printf(0, py, 0xffff0000, "Frames per second %0.2f", Game::fps);
	py += print(0, py, 0xffff0000, "Init First Level Status");
#endif
...
void InitFirstLevelState::onLoad( ) {
	// Sound
	SoundPlayer::get( )->playStream( "first_level" );

	// Brain
	BrainManager::get( )->setCurrentBrain( 1 );
	...
	// Inicializar logica
	getFirstLevelLogic( ).initLevel( );
}

void InitFirstLevelState::onUnload( ) {
}

void InitFirstLevelState::loadGameState( ) {
	// Vignetting
